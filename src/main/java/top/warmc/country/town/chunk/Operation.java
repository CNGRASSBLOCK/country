...
public class Operation {
	public static void Statement(Player player, boolean isAuto) {
        ...
        if (town == null && !isAuto) { player.displayClientMessage(Component.literal("§6[Country]§3[player]§c您没有建立城镇!"), false); return; }

        if (CountryPool.getTownFromLand(player.level(), player.level().getChunk(player.getOnPos())) != null && !isAuto) { player.displayClientMessage(Component.literal("§6[Country]§a[chunk]§c该区块已被声明!"), false); return; }
