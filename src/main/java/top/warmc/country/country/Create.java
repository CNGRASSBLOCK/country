...
public class Create {
	public static void Create(LevelAccessor world, CommandContext<CommandSourceStack> arguments, Player player) {
		...
		if (CountryPool.isInCountry(player)) { player.displayClientMessage(Component.literal("§6[Country]§3[player]§c您已在某个国家内!"), false); return; }

		String country_name = StringArgumentType.getString(arguments, "country_name");
		if (country_name.isEmpty()) { player.displayClientMessage(Component.literal("§6[Country]§3[player]§c国家名字不能为空!"), false); return; }
		if (CountryPool.has(country_name)) { player.displayClientMessage(Component.literal("§6[Country]§3[player]§c存在同名国家!"), false); return; }
		...
		for (int x = 0; x < 5; x++) {
			for (int z = 0; z < 5; z++) {
				...
				if (!CountryPool.isInCountry(player.level(), chunkAccess)) { town.getLand().add(player.level(), chunkAccess); add_chuck = true; }
			}
		}
